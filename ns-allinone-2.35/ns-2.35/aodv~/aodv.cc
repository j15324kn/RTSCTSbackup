...
AODV::AODV(nsaddr_t id) : Agent(PT_AODV),
  ...
  logtarget = 0;
  ifqueue = 0;
thisnode = (MobileNode *)(Node::get_node_by_address(id)); //idはindexの別名であり、AODVのIPアドレスをあらわす
}
...
#endif      
printf("180:Dst - %d, failed local repair\n", rt->rt_dst);
    }
...
my_energy=thisnode->energy_model()->energy(); // バッテリ残量を取得する
 ...
 //printf("[%4.5f](sec)_______________Node[%d]がNode[%d]からRREQを重複受信しました。このRREQ中の最低バッテリ残量値[%4.5f]は現在の最大値[%4.5f]よりも好条件である。\n",CURRENT_TIME,index, ih->saddr(),rq->rq_min_energy,dst_min_dst_min_energy[rq->rq_src][rq->rq_bcast_id] = rq->rq_min_energy); // 現在までの経路最小バッテリ値の最大値を更新する
 //printf("[%4.5f](sec)_____________Node[%d]です。送信予定のRREPをキャンセルします。(キャンセルパケットのアドレス=[%d]　([rq_src]= [%d], [rq_bcast_id]=[%d]))\n",CURRENT_TIME, index,dst_pkt_ptr[rq->rq_src][rq->rq_bcast_id], ListScheduler::instance().cancel( dst_pkt_ptr[rq->rq_src][rq->rq_bcast_id] )); // すでにスケジューリング済みのRREPパケットをキャンセルする！！！！
...
printf("[%4.5f](sec)_______________Node[%d]が受信しました。Node[%d]より、-はじめての-RREQを受信しました。現在の経路中の最低バッテリ残量値は[%4.5f]です。\n",CURRENT_TIME,index,
ih->saddr(),rq->rq_min_energy);dst_min_energy[rq->rq_src][rq->rq_bcast_id] = rq->rq_min_energy; // 経路の最低バッテリ値rq_min_energyを保存
// ※ dst_min_energy[ ][ ]は、ノード[rq->rq_src]から送信された[rq->rq_bcast_id]番目のRREQについて、現在到着した中で、最低バッテリー値のもっとも大きい経路の最低バッテリー値を格納する変数
...
printf("937:%s(%f): %d\t(%d\t%u\t%d)\t(%d\t%u\t%d)\n",
__FUNCTION__,CURRENT_TIME,index, rt->rt_dst, rt->rt_seqno,rt->rt_nexthop,
...
struct hdr_aodv_request *rq = HDR_AODV_REQUEST(p); // ポインタrqを通してRREQヘッダにアクセスできるようにする
my_energy=thisnode->energy_model()->energy(); // 自分のバッテリ残量値を取得する
...
if(rq->rq_min_energy> my_energy){ // もしも自分のバッテリ残量が、現在のRREQ中の最小バッテリ残量値よりも少なければ、
  rq->rq_min_energy = my_energy; // RREQ内のrq_min_energyフィールドを自身のバッテリ残量値で更新する。
} printf("[%4.5f](sec)_______________Node[%d]がRREQを中継します。現在の経路中の最低バッテリ残量値は[%4.5f]です。\n",CURRENT_TIME, index,rq->rq_min_energy);

     if(delay == 1.0){
     Scheduler::instance().schedule(target_, p,
      				   0.01 * Random::uniform());
   } else {
     if(delay > 0.0) {    //forward関数の実行時に、引数に任意の正の実数delayが与えられたとき
     Scheduler::instance().schedule(target_, p, delay);  // No jitter
   }
 }
...
my_energy=thisnode->energy_model()->energy(); // ノードのもつバッテリー残量を取得する
rq->rq_min_energy = my_energy; // 経路中の最小バッテリとして自身のバッテリ残量を格納
...
AODV::sendReply(nsaddr_t ipdst, u_int32_t hop_count, nsaddr_t rpdst,
                u_int32_t rpseq, u_int32_t lifetime, double timestamp,int rq_src, int rq_bcast_id, double dst_wait) {
Packet *p = Packet::alloc();
struct hdr_cmn *ch = HDR_CMN(p);
struct hdr_ip *ih = HDR_IP(p);
struct hdr_aodv_reply *rp = HDR_AODV_REPLY(p);
...
dst_pkt_ptr[rq_src][rq_bcast_id] = p; //スケジューリングしたrrepパケットアドレスの保存
...
printf("1211:setudan[time:%.5f]\t[id:%d]\n",CURRENT_TIME, index);

#ifdef ERROR
fprintf(stderr, "sending Error from %d at %.2f\n", index, Scheduler::instance().clock());
#endif // DEBUG
printf("1216:sending Error from %d at %.2f\n", index, Scheduler::instance().clock());
