...
//	ssid_ = "0";
	...
	//-----------newRTSThresh---------------
	tcl.evalf("Mac/802_11 set newRTSThresh_");
	if (strcmp(tcl.result(), "0") != 0)
		bind("newRTSThresh_", &newRTSThresh_);
...
Mac802_11::recv_timer()
{
	...
	if(dst != (u_int32_t)index_) {
		//AddStart 2017/01/06
		//RTSを受け取った端末が、宛先端末でない場合に、NAVを設定している？
		//dst:パケットに記載された宛先ノードID　？　index_:実際にパケットを受け取った端末？
		
		//本来の宛先に送る場合のRTS電波強度のしきい値と、RTSを受け取ったときの電波強度を比較し、そのしきい値以上であれば、NAVを設定する。
		double tmp_threshold=10000;
		if( dst != -1){
		for( int i=0; i < 1000 ; i++) {
			if( pktRx_->txinfo_.getNode()->nlist[i].nodeid == dst){
				tmp_threshold = pktRx_->txinfo_.getNode()->nlist[i].distance;
				//printf("Nlist_id:%d ",pktRx_->txinfo_.getNode()->nlist[i].nodeid);
				break;
			}
			if( pktRx_->txinfo_.getNode()->nlist[i].nodeid == -1){
				break;
			}
		}
		}
		//printf("Stype:%d ",subtype);//mac-802_11.hに定義あり
		
		//if( dst != -1 ) printf("T:%f S:%d D:%d R:%d tmp:%e RxPr:%e ", Scheduler::instance().clock(),pktRx_->txinfo_.getNode()->nodeid(), dst, index_,tmp_threshold,pktRx_->txinfo_.RxPr); //追加SetNAV
		if ( pktRx_->txinfo_.RxPr > tmp_threshold ) {
			//if( dst != -1 ) printf("SetNAV!\n"); //追加SetNav
			set_nav(mh->dh_duration);     //ここのset_navでノードのnav期間を設定している。
//printf("NAV開始時間[time%f]%f\n",Scheduler::instance().clock(),nav_);
//printf("NAV経過時間[time%f]%.20f\n",Scheduler::instance().clock(),(nav_ - Scheduler::instance().clock()));
//printf("bbbbb3[time%f]%e\n",Scheduler::instance().clock(),nav_);
//		printf("T:%f S:%d D:%d R:%d tmp:%e RxPr:%e [dura:%f]\n", Scheduler::instance().clock(),pktRx_->txinfo_.getNode()->nodeid(), dst, index_,tmp_threshold,pktRx_->txinfo_.RxPr,mh->dh_duration);
		}else{
			//if( dst != -1 ) printf("NotNAV\n"); //nagumo
		}
	}
	 ...
	 * I can hear the packet, the src is my neighbor
	   パケットをフィルタする前に、
	   パケットを聞くことができる隣接ノードを記録します。srcは私の隣人です
	 ...
	 * Address Filtering
	 */
	 //RTSの本来の送信先と、そのRTSを受信したノードが異なる場合は
	 //recvRTSが呼ばれない？
	...
	if(dst != (u_int32_t)index_ && dst != MAC_BROADCAST) {
		 ...
		 *  the packet instead of calling the drop routine.
		    このイベントをログに記録したくないので、
		    ドロップルーチンを呼び出す代わりにパケットを解放するだけです。
		 */
		discard(pktRx_, "---");
		goto done;
	}
...
{
	struct rts_frame *rf = (struct rts_frame*)p->access(hdr_mac::offset_);
	//printf("getRTSThreshold : %e	getnewRTS : %e\r\n",macmib_.getRTSThreshold(),macmib_.getnewRTSTh());
	if(tx_state_ != MAC_IDLE) {
	...
	//AddStart 2016/11/24
	//受け取ったRTSの電波強度が、ノードごとに設定したしきい値以下であれば
	//それ以降この関数での処理は行わず、returnする
	
	//int Src = p->txinfo_.getNode()->nodeid();
	int Dst = addr();
	//デバッグ用コンソール出力
	//struct hdr_cmn *ch = HDR_CMN(p);
	//printf("T:%f ",Scheduler::instance().clock());
	//printf("Src:%d ",p->txinfo_.getNode()->nodeid());//送信元
	//printf("Dst:%d ",addr());//受信先
	/*
	if(p->txinfo_.RxPr < p->txinfo_.getNode()->newRTSThreshForNode() ){
		//printf("Drop!RxRr:%e RTSfN:%e\n",p->txinfo_.RxPr,p->txinfo_.getNode()->newRTSThreshForNode());
		printf("RTS Drop!!\n");
		discard(p, DROP_MAC_BUSY);
		//iscard(p, DROP_MAC_SLEEP);
		Packet::free(p);
		return;
	}else{
		printf("Receiving Sucsess!\n");
	}*/
	
	//AddStart 2017/01/05
	//送信元のネイバーリストから、送信先の電波強度の値を取ってくる
	double tmp_threshold=10000;
	for( int i=0; i < 1000 ; i++) {
		if( p->txinfo_.getNode()->nlist[i].nodeid == Dst){
			tmp_threshold = p->txinfo_.getNode()->nlist[i].distance;
			break;
		}
		if( p->txinfo_.getNode()->nlist[i].nodeid == -1){
			//printf("Dst %d is Not Exist... ",Dst);
			break;
		}
	}
//	printf("tmpTh:%e RxPr:%e ",tmp_threshold,p->txinfo_.RxPr); //komenntoauto nagumo
	if( p->txinfo_.RxPr < tmp_threshold ){
	//旧仕様でのシミュレーション用
//	if( p->txinfo_.RxPr < newRTSThresh_ ){	//Drop
	//	printf("RTSdrop!\n");
		discard(p, DROP_MAC_BUSY);
		Packet::free(p);
		return;
